...
class AimbotConfigV61:
    ...
    def version(self) -> str:
        return AIMBOT_V61_VERSION

    @classmethod
    def from_json(cls, path: str | Path) -> "AimbotConfigV61":
    ...
    def to_json(self, path: str | Path):
        data: Dict[str, Any] = {
            "_comment": "macos-dualbox-aim V6.1.0 - standalone CoreML runtime, MPID control, adaptive integral gate, and multi-object tracker target selection",
            "_version": AIMBOT_V61_VERSION,
            "_description": "capture card -> CoreML detections -> Kalman+Hungarian tracks -> learned MPID with near-target integral gate -> KMBox",
        }
        output = Path(path)
        if output.exists():
            try:
                ...
                pass
        data["_version"] = AIMBOT_V61_VERSION
