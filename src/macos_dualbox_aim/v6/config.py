...
class AimbotConfigV6:
    ...
    def version(self) -> str:
        return AIMBOT_V6_VERSION

    @classmethod
    def from_json(cls, path: str | Path) -> "AimbotConfigV6":
    ...
    def to_json(self, path: str | Path):
        data: Dict[str, Any] = {
            "_comment": "macos-dualbox-aim V6.0.0 - standalone CoreML runtime, MPID control, and multi-object tracker target selection",
            "_version": AIMBOT_V6_VERSION,
            "_description": "capture card -> CoreML detections -> Kalman+Hungarian tracks -> learned MPID -> KMBox",
        }
        output = Path(path)
        if output.exists():
            try:
                ...
                pass
        data["_version"] = AIMBOT_V6_VERSION
