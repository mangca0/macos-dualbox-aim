...
class AimbotConfigV64:
    ...
    stop_brake_enabled: bool = True
    stop_brake_radius: float = 18.0
    stop_brake_output_decay: float = 0.35
    stop_brake_pred_decay: float = 0.2
    stop_brake_min_output: float = 35.0
    ...
    def version(self) -> str:
        return AIMBOT_V64_VERSION

    @classmethod
    def from_json(cls, path: str | Path) -> "AimbotConfigV64":
    ...
    def validate(self):
        ...
        if self.class_names is not None and len(self.class_names) != self.class_count:
            raise ValueError("class_names length must match class_count")
        if self.selected_class_ids is None:
            self.selected_class_ids = list(range(self.class_count))
        normalized_ids: list[int] = []
        for value in self.selected_class_ids:
            if isinstance(value, bool) or not isinstance(value, int):
                raise ValueError("selected_class_ids must contain integers")
            if not 0 <= value < self.class_count:
                raise ValueError("selected_class_ids entries must be between 0 and class_count - 1")
            if value not in normalized_ids:
                normalized_ids.append(value)
        self.selected_class_ids = normalized_ids
        if self.class_names is None:
            self.class_names = [f"class_{index}" for index in range(self.class_count)]
        else:
            normalized_names = []
            for value in self.class_names:
        ...
        if self.stop_brake_radius < 0.0:
            raise ValueError("stop_brake_radius must be zero or positive")
        if not 0.0 <= self.stop_brake_output_decay <= 1.0:
            raise ValueError("stop_brake_output_decay must be between 0.0 and 1.0")
        if not 0.0 <= self.stop_brake_pred_decay <= 1.0:
            raise ValueError("stop_brake_pred_decay must be between 0.0 and 1.0")
        if self.stop_brake_min_output < 0.0:
            raise ValueError("stop_brake_min_output must be zero or positive")
    ...
    def to_json(self, path: str | Path):
        data: Dict[str, Any] = {
            "_comment": "macos-dualbox-aim V6.4.0 - V6.3 plus stop-brake anti-overshoot control in tuner",
            "_version": AIMBOT_V64_VERSION,
            "_description": "capture card -> CoreML detections filtered by selected model classes -> crosshair color detection -> target relative to crosshair -> learned MPID -> KMBox",
        }
        output = Path(path)
        if output.exists():
            try:
                ...
                pass
        data["_version"] = AIMBOT_V64_VERSION
...
def _coerce_config_value(key: str, value: Any) -> Any:
    if key == "class_names":
        return _as_str_list(key, value)
    if key == "selected_class_ids":
        return _as_int_list(key, value)
...
def _as_int_list(key: str, value: Any) -> list[int]:
