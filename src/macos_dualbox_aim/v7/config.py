...
class AimbotConfigV7:
    ...
    noise_amp: float = 0.0
    pid_integral_gate_enabled: bool = False
    ...
    def version(self) -> str:
        return AIMBOT_V7_VERSION

    @classmethod
    def from_json(cls, path: str | Path) -> "AimbotConfigV7":
    ...
    def validate(self):
        ...
        if self.noise_amp < 0.0:
            raise ValueError("noise_amp must be zero or positive")
    ...
    def to_json(self, path: str | Path):
        data: Dict[str, Any] = {
            "_comment": "macos-dualbox-aim V7.0.0 - V6.3 real chain with strict incremental PID learning-controller replica",
            "_version": AIMBOT_V7_VERSION,
            "_description": "capture card -> CoreML detections filtered by selected model classes -> crosshair color detection -> target relative to crosshair -> strict incremental PID replica -> KMBox",
        }
        output = Path(path)
        if output.exists():
            try:
                ...
                pass
        data["_version"] = AIMBOT_V7_VERSION
