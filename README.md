...
V1 默认配置在 `configs/config_v1.json`；V2 默认配置在 `configs/config_v2.json`。
主程序运行后默认在 `http://127.0.0.1:8765` 提供对应版本 tuner。
...
当前 V1 实现在 `src/macos_dualbox_aim/v1/`。V2 实现在
`src/macos_dualbox_aim/v2/`，在 V1 链路基础上加入目标状态 Kalman 滤波。
`scripts/main_v1.py` 和 `scripts/main_v2.py` 都显式导入对应版本路径，避免
V2 实验影响 V1 主链路。测试也应显式导入对应 major 版本路径。
