# macos-dualbox-aim V1.2.1

V1.2.1 是基于 V1.0.0 行为链路的延迟观测和采集负载实验版本，只保留：
...
V1.2.1 保留 V1.0.0 的运行链路行为，并在 V1.2.0 的独立采集模式 probe 上，新增可选负载模拟，用于判断主链路推理节拍是否会让采集 `grab/retrieve/frame_interval` 退化：
...
for i in 1 2 3 4 5; do
  uv run python scripts/latency_tool.py capture --label v1.2.1 --run run$i --duration 60 --interval 0.5
done

uv run python scripts/latency_tool.py compare "latency_runs/*.jsonl" --baseline-label v1.0.0 --candidate-label v1.2.1 --out latency_runs/v1.0.0_vs_v1.2.1.md
```
...
uv run python scripts/capture_probe.py \
  ...
  --warmup 20 \
  --backend auto \
  --out-jsonl latency_runs/capture_probe_v1.2.1.jsonl \
  --out-md latency_runs/capture_probe_v1.2.1.md
```
...
用 `--load sleep --load-ms 9` 模拟每帧推理消费约 9ms 的节拍；用 `--load busy --load-ms 9` 粗略模拟同进程 CPU 调度压力。二者都只影响 probe，不会修改主程序配置。
...
uv run python scripts/capture_probe.py \
  ...
  --load sleep \
  --load-ms 9 \
  --out-jsonl latency_runs/capture_probe_v1.2.1_sleep9.jsonl \
  --out-md latency_runs/capture_probe_v1.2.1_sleep9.md
```
