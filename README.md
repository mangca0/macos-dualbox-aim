# macos-dualbox-aim V1.2.2

V1.2.2 是基于 V1.0.0 行为链路的延迟观测和采集负载实验版本，只保留：
...
V1.2.2 保留 V1.0.0 的运行链路行为，并在 V1.2.0/V1.2.1 的独立采集模式 probe 上，新增后台线程负载模式，用于区分串行消费节拍和同进程并发负载是否会让采集 `grab/retrieve/frame_interval` 退化：
...
for i in 1 2 3 4 5; do
  uv run python scripts/latency_tool.py capture --label v1.2.2 --run run$i --duration 60 --interval 0.5
done

uv run python scripts/latency_tool.py compare "latency_runs/*.jsonl" --baseline-label v1.0.0 --candidate-label v1.2.2 --out latency_runs/v1.0.0_vs_v1.2.2.md
```
