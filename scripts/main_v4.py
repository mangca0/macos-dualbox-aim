...
from macos_dualbox_aim.v1 import HotkeyConfig, HotkeyMonitor, RealtimeInference
from macos_dualbox_aim.v4 import AimbotConfigV4, AimbotV4, WebTuner


def main():
    config_path = project_root / "configs" / "config_v4.json"
    try:
        if config_path.exists():
            config = AimbotConfigV4.from_json(config_path)
        else:
    ...
    if config.enable_tuner:
        try:
            ...
            tuner.start()
            print(f"Aimbot V4 tuner running at {tuner.url}")
        except OSError as exc:
            print(f"Aimbot V4 tuner failed to start on {config.tuner_host}:{config.tuner_port}: {exc}")

    print("Aimbot V4 running. Press Ctrl-C to stop.")
    try:
        engine.start()
