...
from macos_dualbox_aim.v62 import AimbotConfigV62, AimbotV62, RealtimeInferenceV62, WebTuner


def main():
    config_path = project_root / "configs" / "config_v62.json"
    try:
        config = load_config(config_path)
    except ValueError as exc:
        print(f"Invalid V6.2 config: {exc}")
        return
    ...
    if config.enable_tuner:
        try:
            ...
            tuner.start()
            print(f"Aimbot V6.2 tuner running at {tuner.url}")
        except OSError as exc:
            print(f"Aimbot V6.2 tuner failed to start on {config.tuner_host}:{config.tuner_port}: {exc}")

    print("Aimbot V6.2 running with crosshair color reference, MPID control, and multi-object tracking. Press Ctrl-C to stop.")
    try:
        engine.start()
...
def load_config(path: Path) -> AimbotConfigV62:
    if path.exists():
        return AimbotConfigV62.from_json(path)
