...
from macos_dualbox_aim.v6 import AimbotConfigV6, AimbotV6, RealtimeInferenceV6, WebTuner


def main():
    config_path = project_root / "configs" / "config_v6.json"
    try:
        config = load_config(config_path)
    except ValueError as exc:
        print(f"Invalid V6 config: {exc}")
        return
    ...
    if config.enable_tuner:
        try:
            ...
            tuner.start()
            print(f"Aimbot V6 tuner running at {tuner.url}")
        except OSError as exc:
            print(f"Aimbot V6 tuner failed to start on {config.tuner_host}:{config.tuner_port}: {exc}")

    print("Aimbot V6 running with standalone Core ML runtime, MPID control, and multi-object tracking. Press Ctrl-C to stop.")
    try:
        engine.start()
...
def load_config(path: Path) -> AimbotConfigV6:
    if path.exists():
        return AimbotConfigV6.from_json(path)
