...
def main():
    config_path = project_root / "configs" / "aimbot_config_v1_0_1.json"
    try:
        if config_path.exists():
            config = AimbotConfigV1.from_json(config_path)
        else:
            config = AimbotConfigV1(frame_queue_size=1)
    ...
    if config.enable_tuner:
        try:
            ...
            tuner.start()
            print(f"Aimbot V1.0.1 latency tuner running at {tuner.url}")
        except OSError as exc:
            print(f"Aimbot V1.0.1 tuner failed to start on {config.tuner_host}:{config.tuner_port}: {exc}")

    print("Aimbot V1.0.1 latency optimization running. Press Ctrl-C to stop.")
    try:
        engine.start()
