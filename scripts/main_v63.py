...
from macos_dualbox_aim.v63 import (
    AimbotConfigV63,
    AimbotV63,
...
def main():
    config_path = project_root / "configs" / "config_v63.json"
    try:
        config = load_config(config_path)
    except ValueError as exc:
        print(f"Invalid V6.3 config: {exc}")
        return
    ...
    if config.enable_tuner:
        try:
            ...
            tuner.start()
            print(f"Aimbot V6.3 tuner running at {tuner.url}")
        except OSError as exc:
            print(f"Aimbot V6.3 tuner failed to start on {config.tuner_host}:{config.tuner_port}: {exc}")

    print("Aimbot V6.3 running with model-aware class filtering, crosshair color reference, MPID control, and multi-object tracking. Press Ctrl-C to stop.")
    try:
        engine.start()
...
def load_config(path: Path) -> AimbotConfigV63:
    if path.exists():
        return AimbotConfigV63.from_json(path)
