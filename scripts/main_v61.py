...
from macos_dualbox_aim.v61 import AimbotConfigV61, AimbotV61, RealtimeInferenceV61, WebTuner


def main():
    config_path = project_root / "configs" / "config_v61.json"
    try:
        config = load_config(config_path)
    except ValueError as exc:
        print(f"Invalid V6.1 config: {exc}")
        return
    ...
    if config.enable_tuner:
        try:
            ...
            tuner.start()
            print(f"Aimbot V6.1 tuner running at {tuner.url}")
        except OSError as exc:
            print(f"Aimbot V6.1 tuner failed to start on {config.tuner_host}:{config.tuner_port}: {exc}")

    print("Aimbot V6.1 running with standalone Core ML runtime, MPID control, and multi-object tracking. Press Ctrl-C to stop.")
    try:
        engine.start()
...
def load_config(path: Path) -> AimbotConfigV61:
    if path.exists():
        return AimbotConfigV61.from_json(path)
