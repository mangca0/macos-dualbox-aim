...
from macos_dualbox_aim.v3 import AimbotConfigV3, AimbotV3, HotkeyConfig, HotkeyMonitor, RealtimeInference, WebTuner


def main():
    config_path = project_root / "configs" / "config_v3.json"
    try:
        if config_path.exists():
            config = AimbotConfigV3.from_json(config_path)
        else:
    ...
    if config.enable_tuner:
        try:
            ...
            tuner.start()
            print(f"Aimbot V3 tuner running at {tuner.url}")
        except OSError as exc:
            print(f"Aimbot V3 tuner failed to start on {config.tuner_host}:{config.tuner_port}: {exc}")

    print("Aimbot V3 running. Press Ctrl-C to stop.")
    try:
        engine.start()
