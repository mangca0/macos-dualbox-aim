...
def main():
    ...
    if config.enable_tuner:
        try:
            ...
            tuner.start()
            print(f"Aimbot V1 tuner running at {tuner.url}")
        except OSError as exc:
            print(f"Aimbot V1 tuner failed to start on {config.tuner_host}:{config.tuner_port}: {exc}")

    print("Aimbot V1 running. Press Ctrl-C to stop.")
    try:
        engine.start()
