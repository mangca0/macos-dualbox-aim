...
from macos_dualbox_aim.v64 import (
    AimbotConfigV64,
    AimbotV64,
...
def main():
    config_path = project_root / "configs" / "config_v64.json"
    try:
        config = load_config(config_path)
    except ValueError as exc:
        print(f"Invalid V6.4 config: {exc}")
        return
    ...
    if config.enable_tuner:
        try:
            ...
            tuner.start()
            print(f"Aimbot V6.4 tuner running at {tuner.url}")
        except OSError as exc:
            print(f"Aimbot V6.4 tuner failed to start on {config.tuner_host}:{config.tuner_port}: {exc}")

    print("Aimbot V6.4 running with model-aware class filtering, crosshair color reference, MPID control, and multi-object tracking. Press Ctrl-C to stop.")
    try:
        engine.start()
...
def load_config(path: Path) -> AimbotConfigV64:
    if path.exists():
        return AimbotConfigV64.from_json(path)
